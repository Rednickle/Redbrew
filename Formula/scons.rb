class Scons < Formula
  ...
  bottle do
    ...
    sha256 "8f2db57a32310db175d458c5aa2317952247c0f9404a27bdb885da7aa4438674" => :x86_64_linux
  end
  ...
  def install
    unless OS.mac?
      ...
      Dir["**/*"].each do |f|
        ...
        inreplace f, %r{#! ?/usr/bin/env python}, "#! /usr/bin/env python3"
      end
    end

    man1.install gzip("scons-time.1", "scons.1", "sconsign.1")
    system (OS.mac? ? "/usr/bin/python" : Formula["python@3.8"].opt_bin/"python3"), "setup.py", "install",
             "--prefix=#{prefix}",
             "--standalone-lib",
